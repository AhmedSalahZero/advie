@section('title') @if(isset($blog->id)) Edit @else Create News @endif  Page @endsection
                    ...
                    <div class="tab-pane active" id="tab_0">
                        <div class="portlet box green">
                            <div class="portlet-title">
                                <div class="caption">
                                    <i class="fa fa-gift"></i>@if(isset($blog->id)) Edit @else Create News @endif </div>
                                <div class="tools">
                            ...
                            <div class="portlet-body form">
                                <!-- BEGIN FORM-->
                                <form action="{{ isset($blog->id) ?  route('blog.update' , [$blog->id]) : route('blog.store') }}" class="form-horizontal" method="post"  enctype="multipart/form-data">
                                    @csrf
                                    @if(isset($blog->id))
                                        @method('PUT')
                                    @endif
                                    <div class="form-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-6">
                                                    <input
                                                        ...
                                                        placeholder="Enter {{$one->name}} News Title"
                                                        @if(isset($blog->id)) value="{{$blog->title["$one->code"]}}" @else value='{{old("title.".$one->code)}}' @endif
                                        ...
                                        @endforeach
                                            @foreach($AllLanguages as $one)
                                                <div class="form-group">
                                                    <label class="col-md-3 control-label">{{$one->name}} small_description
                                                        <span class="required" aria-required="true"> * </span>
                                                    </label>
                                                    <div class="col-md-6">
                                                        <input
                                                            type="text"
                                                            name="small_description[{{$one->code}}]"
                                                            class="form-control input-circle"
                                                            placeholder="Enter {{$one->name}} News small_description"
                                                            @if(isset($blog->id)) value="{{$blog->small_description["$one->code"]}}" @else value='{{old("small_description.".$one->code)}}' @endif
                                                            @if($errors->first("small_description.".$one->code)) style="border: solid 1px red;" @endif
                                                        >
                                                        <span style="color: red;">{{str_replace("."," ",$errors->first("small_description.".$one->code)) }}</span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        @foreach($AllLanguages as $one)
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-8">
                                                    <textarea class="ckeditor form-control" id="content_{{$one->code}}" name="content[{{$one->code}}]"
                                                        ...
                                                        @if(isset($blog->id)) {{$blog->content[$one->code]}} @else {{old("content.".$one->code)}} @endif
                                                    </textarea>
                                                ...
                                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                                    ...
                                                    <span class="btn default btn-file">
                                                      <input
                                                          ...
                                                          type="file"
                                                          name="image"
                                                          @if(isset($blog->id)) value="{{$blog->image}}"  @endif
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <span class="btn default btn-file">
                                                  <input
                                                      ...
                                                      type="file"
                                                      name="banner"
                                                      @if(isset($blog->id)) value="{{$blog->banner}}" @else value="{{@old('banner')}}" @endif
                                    ...
                                    <div class="form-actions">
                                        <div class="row">
                                            <div class="col-md-offset-3 col-md-9">
                                                ...
                                                <a href="{{route('blog.index')}}" class="btn btn-circle grey-salsa btn-outline">Cancel</a>
                                            </div>
                                        </div>
                                    </div>
                                </form>
